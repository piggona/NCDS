...
def fetch_old_user(start_time,user_time_range):
    conn = pymysql.connect(host='127.0.0.1',port=3306,user="jinyuanhao",db="infomation",passwd="Sjk0213%$")
    cursor = conn.cursor()
    user_query = "SELECT user_id from aliyun_user_info Where register_time < "+str(start_time-user_time_range)
    # print("user_query:{}".format(user_query))
...
def get_new_user_ctr(start_time,user_time_range):
    ...
    for user_id in fetch_new_user(start_time,user_time_range):
        ...
        first_query = "SELECT DISTINCT(item_id,bhv_type) FROM aliyun_behavior_info WHERE user_id = {0} ORDER BY bhv_time ASC LIMIT {1}".format("'"+str(user_id)+"'",str(data_scan_amount))
